...
- `attacker` — отдельная Linux-машина для приёма reverse-shell/reverse TCP коннектов в лаборатории (при старте автоматически устанавливается Responder: через apt, через apk, либо из исходников как fallback).
```bash
