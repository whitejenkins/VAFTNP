...
- `attacker` — отдельная Linux-машина для приёма reverse-shell/reverse TCP коннектов в лаборатории (при старте контейнера выполняется установка `responder` через `apt`).
```bash
